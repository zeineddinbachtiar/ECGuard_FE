...
import ECGChart from './components/ECGCharts'; // Import ECGChart
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchRoleAndData = async () => {
      try {
        const [role, statistics] = await Promise.all([checkRole(), getStatictics()]);
        setRole(role);
        setData(statistics);
      } catch (error) {
      ...
      <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
        ...
        {/* <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-12">
          <Card className="col-span-4">
            <CardHeader>
              <CardTitle>
                <Badge variant="secondary">new</Badge> Recent Registered
              </CardTitle>
            </CardHeader>
            <CardContent>
